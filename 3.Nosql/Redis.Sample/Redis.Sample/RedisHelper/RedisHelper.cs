...
namespace Redis.Sample.RedisHelper
{
    public class RedisHelper
    {
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public bool LockTake(string key, string value, TimeSpan expiry = default(TimeSpan))
        {
            key = AddSysCustomKey(key);
        ...
        /// <param name="expiry"></param>
        /// <returns></returns>
        public async Task<bool> LockTakeAsync(string key, string value, TimeSpan expiry = default(TimeSpan))
        {
            key = AddSysCustomKey(key);
            return await Do(db => db.LockTakeAsync(key, value, expiry));
        }
        ...
        {
            key = AddSysCustomKey(key);
            return await Do(db => db.LockReleaseAsync(key, value));
        }
